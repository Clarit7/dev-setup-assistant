...
| **보안 검사** | 모든 명령어를 화이트리스트 + 블랙리스트 이중 검사 후 실행, 화이트리스트에 없는 명령어는 LLM 동적 안전성 검사 수행, API 키·DB 비밀번호는 허용 키 화이트리스트로 추가 검증 |
...
- `SAFE`: 동적 화이트리스트에 추가 후 실행
- `CAUTION`: 사용자 확인 다이얼로그 표시
- `DANGEROUS`: 차단
